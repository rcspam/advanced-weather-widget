...
ColumnLayout {
    ...
    readonly property font ttFont: {
        if (!Plasmoid.configuration.tooltipUseSystemFont
                && (Plasmoid.configuration.tooltipFontFamily || "").length > 0) {
            return Qt.font({
    ...
    function getTooltipCustomIcon(itemId) {
        ...
        raw.split(";").forEach(function(pair) {
            var kv = pair.split("=")
            if (kv.length === 2 && kv[0].trim().length > 0) m[kv[0].trim()] = kv[1].trim()
        })
    ...
    function ttItemIconInfo(tok) {
        ...
        if (theme === "wi-font") {
            ...
            var glyphs = {
                temperature: "\uF055", feelslike: "\uF05D", condition: "\uF013",
                wind: "\uF050", humidity: "\uF07A", pressure: "\uF079",
                dewpoint: "\uF055", visibility: "\uF0B6",
                moonphase: Moon.moonPhaseGlyph(),
                "suntimes-sunrise": "\uF051", "suntimes-sunset": "\uF052"
            }
            return { type: "wi", source: glyphs[tok] || "", kdeFallback: "" }
        }

        if (theme === "custom") {
            var defaults = {
                temperature: "thermometer",   feelslike: "thermometer",
                condition:   W.weatherCodeToIcon(weatherRoot ? weatherRoot.weatherCode : -1,
                             weatherRoot ? weatherRoot.isNightTime() : false),
                wind:        "weather-windy", humidity: "weather-showers",
                pressure:    "weather-overcast", dewpoint: "thermometer",
                visibility:  "weather-fog",   moonphase: "weather-clear-night",
                "suntimes-sunrise": "weather-sunrise", "suntimes-sunset": "weather-sunset"
            }
            var saved = getTooltipCustomIcon(tok)
            return { type: "kde", source: saved.length > 0 ? saved : (defaults[tok] || ""), kdeFallback: "" }
        }

        if (theme === "kde") {
            var kdeMap = {
                temperature: "thermometer",        feelslike: "thermometer",
                wind:        "weather-wind-beaufort-0", humidity: "weather-humidity",
                pressure:    "weather-pressure",   dewpoint: "thermometer",
                visibility:  "weather-fog",        moonphase: "weather-clear-night",
                "suntimes-sunrise": "weather-sunrise", "suntimes-sunset": "weather-sunset"
            }
            if (tok === "condition")
                return { type: "kde", source: weatherRoot
                    ? W.weatherCodeToIcon(weatherRoot.weatherCode, weatherRoot.isNightTime())
                    : "weather-none-available", kdeFallback: "" }
            return { type: "kde", source: kdeMap[tok] || "", kdeFallback: "" }
        }
        ...
        var svgMap = {
            temperature: "thermometer", feelslike: "thermometer",
            humidity: "humidity", pressure: "barometer", wind: "strong-wind",
            dewpoint: "thermometer", visibility: "thermometer",
            moonphase: Moon.moonPhaseSvgStem(),
            "suntimes-sunrise": "sunrise", "suntimes-sunset": "sunset"
        }
        if (tok === "condition") {
            if (!weatherRoot) return { type: "svg", source: "", kdeFallback: "" }
            ...
            return { type: "svg", source: base + stem + ".svg", kdeFallback: "" }
        }
        var s = svgMap[tok]
        // wi-sunrise.svg and wi-sunset.svg exist in all SVG theme packs — no kdeFallback needed
        return { type: "svg", source: s ? base + s + ".svg" : "", kdeFallback: "" }
    }

    // ── Header: location name ─────────────────────────────────────────────
    Label {
        Layout.fillWidth: true
        text: weatherRoot && weatherRoot.hasSelectedTown
    ...
    GridLayout {
        ...
        Repeater {
            model: {
                if (!weatherRoot || !ttUseIcons) return []
                var _ = weatherRoot.temperatureC + weatherRoot.windKmh
                      ...
                      + ttIconTheme + ttIconSize + ttSunTimesMode
            ...
            delegate: RowLayout {
                ...
                // ── wi-font glyph ─────────────────────────────────────────
                Text {
                    visible: modelData.showIcon && modelData.glyphType === "wi"
                ...
                // ── KDE icon ──────────────────────────────────────────────
                Kirigami.Icon {
                    visible: modelData.showIcon && modelData.glyphType === "kde"
                          && (modelData.glyph || "").length > 0
                ...
                // ── SVG icon ──────────────────────────────────────────────
                Item {
                    visible: modelData.showIcon && modelData.glyphType === "svg"
                          && (modelData.glyph || "").length > 0
                    implicitWidth:  ttRoot.ttIconSize
                    implicitHeight: ttRoot.ttIconSize
                    Layout.alignment: Qt.AlignVCenter
    ...
    function _buildTooltipItems() {
        if (!weatherRoot || !weatherRoot.hasSelectedTown) return []
        var iconMap = {}
        var raw = Plasmoid.configuration.tooltipItemIcons || ""
        if (raw.length > 0) {
            raw.split(";").forEach(function(pair) {
                var kv = pair.split("=")
                if (kv.length === 2) iconMap[kv[0].trim()] = (kv[1].trim() === "1")
            })
        }
        var order = (Plasmoid.configuration.tooltipItemOrder
                  || "temperature;wind;humidity;pressure;suntimes")
            .split(";").filter(function(t) { return t.trim().length > 0 })
        var rows = []
        order.forEach(function(tok) {
            _tooltipItemsFor(tok.trim(), iconMap).forEach(function(row) { rows.push(row) })
        })
        return rows
    }
    ...
    function _tooltipItemsFor(tok, iconMap) {
        ...
        function iconRow(iconTok, txt) {
            ...
            return { glyph: info.source, glyphType: info.type, glyphKdeFallback: info.kdeFallback,
                     showIcon: showIcon, text: txt }
        }
        function textRow(txt) {
            return { glyph: "", glyphType: "", glyphKdeFallback: "", showIcon: false, text: txt }
        }
        ...
        if (tok === "temperature")
            return [row("temperature", r.tempValue(r.temperatureC),
                        i18n("Temperature:") + " " + r.tempValue(r.temperatureC))]

        if (tok === "feelslike")
            return [row("feelslike", r.tempValue(r.apparentC),
                        i18n("Feels like:") + " " + r.tempValue(r.apparentC))]

        if (tok === "condition")
            return [row("condition", r.weatherCodeToText(r.weatherCode, r.isNightTime()),
                        i18n("Condition:") + " " + r.weatherCodeToText(r.weatherCode, r.isNightTime()))]

        if (tok === "wind") {
            var windTxt = r.windValue(r.windKmh)
            if (textMode) return [textRow(i18n("Wind:") + " " + windTxt)]
            ...
            if (ttIconTheme === "wi-font") {
                ...
                var info = ttItemIconInfo("wind"); glyph = info.source; type = info.type; kdf = info.kdeFallback
            }
            return [{ glyph: glyph, glyphType: type, glyphKdeFallback: kdf, showIcon: showIcon, text: windTxt }]
        }

        if (tok === "humidity")
            return [row("humidity",
                        isNaN(r.humidityPercent) ? "--" : r.humidityPercent.toFixed(1) + "%",
                        i18n("Humidity:") + " " + (isNaN(r.humidityPercent) ? "--" : r.humidityPercent.toFixed(1) + "%"))]

        if (tok === "pressure")
            return [row("pressure", r.pressureValue(r.pressureHpa),
                        i18n("Pressure:") + " " + r.pressureValue(r.pressureHpa))]

        if (tok === "dewpoint")
            return [row("dewpoint", r.tempValue(r.dewPointC),
                        i18n("Dew point:") + " " + r.tempValue(r.dewPointC))]
        ...
        if (tok === "moonphase")
            return [row("moonphase", i18n(Moon.moonPhaseNameKey()),
                        i18n("Moon:") + " " + i18n(Moon.moonPhaseNameKey()))]
        ...
        if (tok === "suntimes") {
            ...
            if (mode === "sunrise") {
                return textMode
                    ? [textRow(i18n("Sunrise:") + " " + riseTime)]
                    : [{ glyph: infoRise.source, glyphType: infoRise.type, glyphKdeFallback: infoRise.kdeFallback,
                         showIcon: showIcon, text: riseTime }]
            }
            if (mode === "sunset") {
                return textMode
                    ? [textRow(i18n("Sunset:") + " " + setTime)]
                    : [{ glyph: infoSet.source, glyphType: infoSet.type, glyphKdeFallback: infoSet.kdeFallback,
                         showIcon: showIcon, text: setTime }]
            }
            if (mode === "upcoming") {
