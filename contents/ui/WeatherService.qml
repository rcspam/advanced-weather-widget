...
QtObject {
    ...
    function _owKey() { return (Plasmoid.configuration.owApiKey || "").trim() }
    function _waKey() { return (Plasmoid.configuration.waApiKey || "").trim() }
    ...
    function refreshNow() {
        ...
        if (!r.hasSelectedTown) {
            r.loading = false; r.updateText = "";
            r.temperatureC = NaN; r.apparentC = NaN;
            r.windKmh = NaN;  r.windDirection = NaN;
            r.pressureHpa = NaN; r.humidityPercent = NaN;
            r.visibilityKm = NaN; r.dewPointC = NaN;
            r.sunriseTimeText = "--"; r.sunsetTimeText = "--";
            r.weatherCode = -1; r.isDay = -1; r.dailyData = []; r.hourlyData = [];
            return;
        }
        r.loading = true;
    ...
    function fetchHourlyForDate(dateStr) {
        ...
        if (ap === "openMeteo")   { _hourlyOpenMeteo(dateStr);  return; }
        if (ap === "openWeather") { _hourlyOpenWeather(dateStr); return; }
        if (ap === "weatherApi")  { _hourlyWeatherApi(dateStr);  return; }
        if (ap === "metno")       { _hourlyMetNo(dateStr);       return; }
    ...
    function _apiTimeTo24h(s) {
        ...
        if (parts.length < 2) return s;  // already "HH:mm" — pass through
        var hm = parts[0].split(":");
        if (hm.length < 2) return s;
    ...
    function _tryProvider(chain, idx) {
        if (idx >= chain.length) {
            ...
            var names = chain.map(function(p) { return _providerLabel(p); });
            weatherRoot.updateText = i18n("Failed: %1").arg(names.join(", "));
            _failed = [];
            return;
        }
        var p = chain[idx];
        if (p === "openWeather") { _currentOpenWeather(chain, idx); return; }
        if (p === "weatherApi")  { _currentWeatherApi(chain, idx);  return; }
        if (p === "metno")       { _currentMetNo(chain, idx);       return; }
    ...
    function _currentOpenMeteo(chain, idx) {
        ...
        req.onreadystatechange = function() {
            ...
            if (req.status !== 200) { _tryProvider(chain, idx + 1); return; }
            var d = JSON.parse(req.responseText);
            if (!d.current) { _tryProvider(chain, idx + 1); return; }
            var c = d.current;
            ...
            if (d.daily && d.daily.time) {
                ...
                for (var i = 0; i < maxD; ++i)
                    nd.push({ day: Qt.formatDate(new Date(d.daily.time[i]), "ddd"),
            ...
            r.dailyData  = nd;
            r.loading    = false;
            r.updateText = i18n("Updated %1 (Open-Meteo)").arg(Qt.formatTime(new Date(), Qt.locale().timeFormat(Locale.ShortFormat)));
        };
        req.send();
    }
    ...
    function _currentOpenWeather(chain, idx) {
        var r = weatherRoot;
        var key = _owKey();
        if (!key) { _tryProvider(chain, idx + 1); return; }
        var baseUrl = "https://api.openweathermap.org/data/2.5/";
        var latlon  = "lat=" + Plasmoid.configuration.latitude
        ...
        req.onreadystatechange = function() {
            ...
            if (req.status !== 200) { _tryProvider(chain, idx + 1); return; }
            var d = JSON.parse(req.responseText);
            if (!d.main) { _tryProvider(chain, idx + 1); return; }
            ...
            fcReq.onreadystatechange = function() {
                if (fcReq.readyState !== XMLHttpRequest.DONE) return;
                var nd = [];
                if (fcReq.status === 200) {
                    ...
                    if (fc.list) fc.list.forEach(function(e) {
                        ...
                        if (!seen[dk] || Math.abs(hr - 12) < Math.abs(seen[dk].hour - 12))
                            seen[dk] = { hour: hr, entry: e, dk: dk };
                    });
                    Object.keys(seen).sort().forEach(function(k) {
                        if (nd.length >= Plasmoid.configuration.forecastDays) return;
                        var e = seen[k].entry;
                        nd.push({ day: Qt.formatDate(new Date(e.dt * 1000), "ddd"), dateStr: k,
                            maxC: e.main.temp_max, minC: e.main.temp_min,
                            code: W.openWeatherCodeToWmo(e.weather[0].id) });
                    });
                }
                r.dailyData  = nd;
                r.loading    = false;
                r.updateText = i18n("Updated %1 (OpenWeather)").arg(Qt.formatTime(new Date(), Qt.locale().timeFormat(Locale.ShortFormat)));
            };
            fcReq.send();
        };
        req.send();
    ...
    function _currentWeatherApi(chain, idx) {
        ...
        var url  = "https://api.weatherapi.com/v1/forecast.json?key=" + encodeURIComponent(key)
        ...
        req.onreadystatechange = function() {
            ...
            if (d.forecast && d.forecast.forecastday) {
                ...
                for (var i = 0; i < maxD; ++i) {
                    ...
                    nd.push({ day: Qt.formatDate(new Date(f.date), "ddd"), dateStr: f.date,
                        maxC: f.day.maxtemp_c, minC: f.day.mintemp_c,
                        code: W.weatherApiCodeToWmo(f.day.condition.code) });
                }
            }
            r.dailyData  = nd;
            r.loading    = false;
            r.updateText = i18n("Updated %1 (WeatherAPI.com)").arg(Qt.formatTime(new Date(), Qt.locale().timeFormat(Locale.ShortFormat)));
        };
        req.send();
    }
    ...
    function _currentMetNo(chain, idx) {
        ...
        var url = "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat="
            ...
            + "&lon=" + encodeURIComponent(Plasmoid.configuration.longitude)
        ...
        req.onreadystatechange = function() {
            ...
            d.properties.timeseries.forEach(function(t2) {
                ...
                if (!det2) return;
                if (!seen[dk] || Math.abs(hr - 12) < Math.abs(seen[dk].hour - 12)) {
                    var s2 = (t2.data && t2.data.next_1_hours && t2.data.next_1_hours.summary)
                        ? t2.data.next_1_hours.summary.symbol_code : "";
                    seen[dk] = { hour: hr, tempC: det2.air_temperature, code: W.metNoSymbolToWmo(s2), dk: dk };
                }
            });
            var nd = [];
            Object.keys(seen).sort().forEach(function(k) {
                if (nd.length >= Plasmoid.configuration.forecastDays) return;
                var s3 = seen[k];
                nd.push({ day: Qt.formatDate(new Date(k), "ddd"), dateStr: k,
                    maxC: s3.tempC, minC: s3.tempC, code: s3.code });
            });
            r.dailyData  = nd;
            r.loading    = false;
            r.updateText = i18n("Updated %1 (met.no)").arg(Qt.formatTime(new Date(), Qt.locale().timeFormat(Locale.ShortFormat)));
        };
        req.send();
    }
        ...
        req.onreadystatechange = function() {
            ...
            if (d.hourly && d.hourly.time)
                for (var i = 0; i < d.hourly.time.length; ++i)
                    arr.push({ hour: Qt.formatTime(new Date(d.hourly.time[i]), "HH:mm"),
    ...
    function _hourlyOpenWeather(dateStr) {
        ...
        var url = "https://api.openweathermap.org/data/2.5/forecast?lat="
            + Plasmoid.configuration.latitude + "&lon=" + Plasmoid.configuration.longitude
            + "&units=metric&appid=" + encodeURIComponent(key);
        var req = new XMLHttpRequest();
        req.open("GET", url);
        req.onreadystatechange = function() {
            ...
            if (fc.list) fc.list.forEach(function(e) {
                ...
                if (Qt.formatDate(d, "yyyy-MM-dd") === dateStr)
                    arr.push({ hour: Qt.formatTime(d, "HH:mm"), tempC: e.main.temp,
                        code:     W.openWeatherCodeToWmo(e.weather[0].id),
                ...
                d.forecast.forecastday.forEach(function(day) {
                    if (day.date === dateStr && day.hour)
                        day.hour.forEach(function(h) {
                            arr.push({ hour: Qt.formatTime(new Date(h.time_epoch * 1000), "HH:mm"),
                                tempC:    h.temp_c,
            ...
            if (d.properties && d.properties.timeseries)
                d.properties.timeseries.forEach(function(ts) {
                    ...
                    if (Qt.formatDate(dd, "yyyy-MM-dd") === dateStr) {
                        ...
                        var sym = (ts.data && ts.data.next_1_hours && ts.data.next_1_hours.summary)
                            ? ts.data.next_1_hours.summary.symbol_code : "";
                        arr.push({ hour: Qt.formatTime(dd, "HH:mm"),
                            ...
                            precipProb: (ts.data && ts.data.next_1_hours && ts.data.next_1_hours.details
