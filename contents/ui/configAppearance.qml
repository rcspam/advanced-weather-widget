...
KCM.AbstractKCM {
    ...
    FontLoader { id: wiFont; source: "../fonts/weathericons-regular-webfont.ttf" }
    ...
    KIconThemes.IconDialog {
        id: iconDialogRise
        onIconNameChanged: {
            if (iconName) root.setCustomIcon("suntimes-sunrise", iconName)
        }
    }
    KIconThemes.IconDialog {
        id: iconDialogSet
        onIconNameChanged: {
            if (iconName) root.setCustomIcon("suntimes-sunset", iconName)
        }
    }
    ...
    KIconThemes.IconDialog {
        ...
        onIconNameChanged: {
            if (iconName) root.setTooltipCustomIcon("suntimes-sunrise", iconName)
        }
    }
    KIconThemes.IconDialog {
        id: iconDialogTooltipSet
        onIconNameChanged: {
            if (iconName) root.setTooltipCustomIcon("suntimes-sunset", iconName)
        }
    }
    ...
    Dialog {
        ...
        readonly property var conditionSlots: [
            ...
            { key: "condition-cloudy-day",    label: i18n("Partly cloudy (day)"),   defaultIcon: "weather-few-clouds"       },
            { key: "condition-cloudy-night",  label: i18n("Partly cloudy (night)"), defaultIcon: "weather-few-clouds-night" },
            { key: "condition-overcast",      label: i18n("Overcast"),              defaultIcon: "weather-overcast"         },
            { key: "condition-fog",           label: i18n("Fog"),                   defaultIcon: "weather-fog"              },
            { key: "condition-rain",          label: i18n("Rain"),                  defaultIcon: "weather-showers"          },
            { key: "condition-snow",          label: i18n("Snow"),                  defaultIcon: "weather-snow"             },
            { key: "condition-storm",         label: i18n("Storm / Thunderstorm"),  defaultIcon: "weather-storm"            }
        ]
        ...
        function _setTempIcon(key, name) {
            var m = {}
            for (var k in _tempMap) if (_tempMap.hasOwnProperty(k)) m[k] = _tempMap[k]
            if (name && name.length > 0) m[key] = name
            else delete m[key]
            _tempMap    = m
        ...
        function _commit() {
            ...
            if (useCustom) {
                ...
                conditionSlots.forEach(function(s) { delete m[s.key] })
            }
            var serialized = root.serializeCustomIcons(m)
            if (context === "tooltip") root.cfg_tooltipCustomIcons = serialized
            else                       root.cfg_panelCustomIcons   = serialized
        }
        ...
        contentItem: ColumnLayout {
            ...
            ColumnLayout {
                ...
                Label { text: i18n("Icon source:"); font.bold: true; opacity: 0.85 }
        ...
        footer: DialogButtonBox {
            Button {
                ...
                onClicked: { conditionIconDialog._commit(); conditionIconDialog.close() }
            }
            Button {
                text: i18n("Cancel")
    ...
    Dialog {
        ...
        function getIcon(id) { return context === "tooltip" ? root.getTooltipCustomIcon(id) : root.getCustomIcon(id) }
        function setIcon(id, name) { if (context === "tooltip") root.setTooltipCustomIcon(id, name); else root.setCustomIcon(id, name) }
        // Which icon strings to watch for reactive re-evaluation
        function watchRaw() { return context === "tooltip" ? root.cfg_tooltipCustomIcons : root.cfg_panelCustomIcons }
        ...
        contentItem: ColumnLayout {
                ...
                RowLayout {
                    ...
                    Button {
                        ...
                        onClicked: {
                            ...
                            if (iconConfigDialog.context === "tooltip") iconDialogTooltipMain.open()
                            else iconDialogMain.open()
                        }
                    }
            ...
            ColumnLayout {
                ...
                // Mode selector
                Label { text: i18n("Display mode:"); font.bold: true; opacity: 0.85 }
                ComboBox {
                    id: sunModeDialogCombo
                    Layout.fillWidth: true
                    textRole: "text"
                    model: [
                        { text: i18n("Upcoming (next sunrise or sunset)"), value: "upcoming" },
                        { text: i18n("Both  07:17 / 18:03"),               value: "both"     },
                        { text: i18n("Sunrise only  07:17"),               value: "sunrise"  },
                        { text: i18n("Sunset only   18:03"),               value: "sunset"   }
                    ]
                    Component.onCompleted: {
                        for (var i = 0; i < model.length; ++i)
                            if (model[i].value === root.cfg_panelSunTimesMode) { currentIndex = i; break }
                    }
                ...
                // Sunrise icon
                Label { text: i18n("Sunrise icon:"); font.bold: true; opacity: 0.85 }
                RowLayout {
                    spacing: Kirigami.Units.smallSpacing
                    Layout.fillWidth: true
                    ...
                    Button {
                        ...
                        icon.name: "document-open"
                        onClicked: {
                            if (iconConfigDialog.context === "tooltip") iconDialogTooltipRise.open()
                            else iconDialogRise.open()
                        }
                    }
                    Button {
                        text: i18n("Reset")
                ...
                // Sunset icon
                Label { text: i18n("Sunset icon:"); font.bold: true; opacity: 0.85 }
                RowLayout {
                    spacing: Kirigami.Units.smallSpacing
                    Layout.fillWidth: true
                    ...
                    Button {
                        ...
                        icon.name: "document-open"
                        onClicked: {
                            if (iconConfigDialog.context === "tooltip") iconDialogTooltipSet.open()
                            else iconDialogSet.open()
                        }
                    }
                    Button {
                        text: i18n("Reset")
    ...
    property bool   cfg_tooltipUseIcons:        true   // true = icons, false = text labels
    ...
    function serializeCustomIcons(map) {
        var parts = [];
        for (var k in map) if (map.hasOwnProperty(k) && map[k].length > 0)
            parts.push(k + "=" + map[k]);
        return parts.join(";");
    }
    function setCustomIcon(itemId, iconName) {
        var m = parseCustomIcons(root.cfg_panelCustomIcons);
        if (iconName.length > 0) m[itemId] = iconName;
    ...
    readonly property var allPanelItemDefs: [
        { itemId: "condition",   label: i18n("Condition"),      description: i18n("Weather icon + condition text"),      wiChar: "\uF00D", iconFallback: "weather-clear"       },
        { itemId: "temperature", label: i18n("Temperature"),    description: i18n("Current temperature"),                wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "suntimes",    label: i18n("Sunrise/Sunset"), description: i18n("Sunrise and sunset times"),           wiChar: "\uF051", iconFallback: "weather-clear-night" },
        { itemId: "wind",        label: i18n("Wind"),           description: i18n("Wind speed and direction"),           wiChar: "\uF059", iconFallback: "weather-windy"       },
        // Issue #3: feelslike now uses F055 (thermometer), not F05D
        { itemId: "feelslike",   label: i18n("Feels Like"),     description: i18n("Apparent (feels-like) temperature"),  wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "humidity",    label: i18n("Humidity"),       description: i18n("Relative humidity percentage"),       wiChar: "\uF07A", iconFallback: "weather-showers"     },
        { itemId: "pressure",    label: i18n("Pressure"),       description: i18n("Atmospheric pressure"),               wiChar: "\uF079", iconFallback: "weather-overcast"    },
        { itemId: "location",    label: i18n("Location Name"),  description: i18n("City / location name"),               wiChar: "\uF0B1", iconFallback: "mark-location"       },
        { itemId: "moonphase",   label: i18n("Moon Phase"),     description: i18n("Current moon phase"),                 wiChar: "\uF0D0", iconFallback: "weather-clear-night" }
    ]
    ...
    readonly property var allDetailsDefs: [
        { itemId: "feelslike",  label: i18n("Feels Like"),    description: i18n("Apparent temperature"),  wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "humidity",   label: i18n("Humidity"),      description: i18n("Relative humidity %"),   wiChar: "\uF07A", iconFallback: "weather-showers"     },
        { itemId: "pressure",   label: i18n("Pressure"),      description: i18n("Atmospheric pressure"),  wiChar: "\uF079", iconFallback: "weather-overcast"    },
        { itemId: "wind",       label: i18n("Wind"),          description: i18n("Wind speed + direction"),wiChar: "\uF050", iconFallback: "weather-windy"       },
        { itemId: "suntimes",   label: i18n("Sunrise/Sunset"),description: i18n("Sun rise & set times"),  wiChar: "\uF051", iconFallback: "weather-clear"       },
        { itemId: "dewpoint",   label: i18n("Dew Point"),     description: i18n("Dew point temperature"), wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "visibility", label: i18n("Visibility"),    description: i18n("Visibility distance"),   wiChar: "\uF0B6", iconFallback: "weather-fog"         },
        { itemId: "moonphase",  label: i18n("Moon Phase"),    description: i18n("Current moon phase"),    wiChar: "\uF0D0", iconFallback: "weather-clear-night" }
    ]
    ...
    readonly property var allTooltipDefs: [
        { itemId: "temperature", label: i18n("Temperature"),    description: i18n("Current temperature"),   wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "condition",   label: i18n("Condition"),      description: i18n("Weather condition text"),wiChar: "\uF013", iconFallback: "weather-few-clouds"  },
        { itemId: "feelslike",   label: i18n("Feels Like"),     description: i18n("Apparent temperature"),  wiChar: "\uF055", iconFallback: "thermometer"         },
        { itemId: "wind",        label: i18n("Wind"),           description: i18n("Wind speed + direction"),wiChar: "\uF050", iconFallback: "weather-windy"       },
        { itemId: "humidity",    label: i18n("Humidity"),       description: i18n("Relative humidity %"),   wiChar: "\uF07A", iconFallback: "weather-showers"     },
        { itemId: "pressure",    label: i18n("Pressure"),       description: i18n("Atmospheric pressure"),  wiChar: "\uF079", iconFallback: "weather-overcast"    },
    ...
    function initPanelModel() {
        ...
        var enabled = cfg_panelItemOrder.split(";").filter(function(t) { return t.trim().length > 0 })
            ...
            for (var j = 0; j < allPanelItemDefs.length; ++j) {
                if (allPanelItemDefs[j].itemId === tok) {
                    panelWorkingModel.append({
                        itemId: allPanelItemDefs[j].itemId, itemLabel: allPanelItemDefs[j].label,
                        itemDesc: allPanelItemDefs[j].description, itemWiChar: allPanelItemDefs[j].wiChar,
                        itemFallback: allPanelItemDefs[j].iconFallback, itemEnabled: true,
        ...
        allPanelItemDefs.forEach(function(def) {
            if (enabled.indexOf(def.itemId) < 0) {
                panelWorkingModel.append({
                    itemId: def.itemId, itemLabel: def.label, itemDesc: def.description,
                    itemWiChar: def.wiChar, itemFallback: def.iconFallback, itemEnabled: false,
    ...
    function firstPanelDisabledIndex() {
        for (var i = 0; i < panelWorkingModel.count; ++i)
            if (!panelWorkingModel.get(i).itemEnabled) return i
        return -1
    }
    function applyPanelItems() {
        ...
        for (var i = 0; i < panelWorkingModel.count; ++i) {
            ...
            if (item.itemEnabled) ids.push(item.itemId)
        }
    ...
    function initDetailsModel() {
        ...
        var enabled = cfg_widgetDetailsOrder.split(";").filter(function(t) { return t.trim().length > 0 })
        enabled.forEach(function(tok) {
            tok = tok.trim()
            for (var j = 0; j < allDetailsDefs.length; ++j) {
                if (allDetailsDefs[j].itemId === tok) {
                    detailsWorkingModel.append({
                        itemId: allDetailsDefs[j].itemId, itemLabel: allDetailsDefs[j].label,
                        itemDesc: allDetailsDefs[j].description, itemEnabled: true,
                        itemWiChar: allDetailsDefs[j].wiChar, itemFallback: allDetailsDefs[j].iconFallback
                    }); break
                }
            }
        })
        allDetailsDefs.forEach(function(def) {
            if (enabled.indexOf(def.itemId) < 0) {
                detailsWorkingModel.append({
                    itemId: def.itemId, itemLabel: def.label, itemDesc: def.description, itemEnabled: false,
    ...
    function applyDetailsItems() {
        ...
        for (var i = 0; i < detailsWorkingModel.count; ++i) {
            if (detailsWorkingModel.get(i).itemEnabled) ids.push(detailsWorkingModel.get(i).itemId)
        }
    ...
    function firstDetailsDisabledIndex() {
        for (var i = 0; i < detailsWorkingModel.count; ++i)
            if (!detailsWorkingModel.get(i).itemEnabled) return i
        return -1
    }
    ...
    function firstTooltipDisabledIndex() {
        for (var i = 0; i < tooltipWorkingModel.count; ++i)
            if (!tooltipWorkingModel.get(i).itemEnabled) return i
        return -1
    }
    function initTooltipModel() {
        tooltipWorkingModel.clear()
        var iconMap = parseTooltipItemIcons()
        var enabled = cfg_tooltipItemOrder.split(";").filter(function(t) { return t.trim().length > 0 })
        enabled.forEach(function(tok) {
            tok = tok.trim()
            for (var j = 0; j < allTooltipDefs.length; ++j) {
                if (allTooltipDefs[j].itemId === tok) {
                    tooltipWorkingModel.append({
                        itemId: allTooltipDefs[j].itemId, itemLabel: allTooltipDefs[j].label,
                        itemDesc: allTooltipDefs[j].description, itemEnabled: true,
                        itemWiChar: allTooltipDefs[j].wiChar, itemFallback: allTooltipDefs[j].iconFallback,
                        itemShowIcon: (tok in iconMap) ? iconMap[tok] : true
                    }); break
                }
            }
    ...
    function setCombo(combo, value) {
        for (var i = 0; i < combo.model.length; ++i)
            if (combo.model[i].value === value) { combo.currentIndex = i; return }
    }
    ...
    header: PlasmaComponents.TabBar {
        ...
        PlasmaComponents.TabButton { icon.name: "view-list-details";              text: i18n("Panel")   }
        PlasmaComponents.TabButton { icon.name: "plasma-symbolic";               text: i18n("Widget")  }
        PlasmaComponents.TabButton { icon.name: "preferences-desktop-feedback";  text: i18n("Tooltip") }
        PlasmaComponents.TabButton { icon.name: "preferences-desktop";           text: i18n("Misc")    }
    }
            ...
            StackLayout {
                ...
                Kirigami.FormLayout {
                    ...
                    ComboBox {
                        ...
                        model: [
                            { text: i18n("Single line (all items at once)"), value: "single"    },
                            { text: i18n("Multiple lines (tall panel)"),     value: "multiline" }
                        ]
                        textRole: "text"
                        // Bug #7 fix: initialize from saved config inside the ComboBox itself.
                        // Component.onCompleted on the root fires before this deferred Component
                        // is instantiated, so root-level setCombo() calls are always no-ops.
                        Component.onCompleted: {
                            for (var i = 0; i < model.length; ++i)
                                if (model[i].value === root.cfg_panelInfoMode) { currentIndex = i; break }
                        }
                        onActivated: root.cfg_panelInfoMode = model[currentIndex].value
                    }

                    // ── Multiple lines options (hidden in Single mode) ─────
                    SpinBox {
                        Kirigami.FormData.label: i18n("Scroll interval (sec):")
                        visible: root.cfg_panelInfoMode === "multiline"
                        from: 1; to: 30; value: root.cfg_panelScrollSeconds
                    ...
                    SpinBox {
                        ...
                        visible: root.cfg_panelInfoMode === "multiline"
                        from: 1; to: 8; value: root.cfg_panelMultiLines
                    ...
                    RowLayout {
                        ...
                        Label { text: i18n("px"); opacity: 0.65 }
                        Label {
                            ...
                            wrapMode: Text.WordWrap; Layout.maximumWidth: 260
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        ComboBox {
                            ...
                            model: [
                                { text: i18n("Bullet  \u2022"),       value: " \u2022 "   },
                                { text: i18n("Pipe  |"),              value: " | "        },
                                { text: i18n("Dash  \u2013"),         value: " \u2013 "   },
                                { text: i18n("Space"),                value: "   "        },
                                { text: i18n("Small circle  \u26ac"), value: " \u26ac "   },
                            ...
                            Component.onCompleted: {
                                ...
                                for (var n = 0; n < model.length - 1; ++n) {
                                    if (model[n].value === root.cfg_panelSeparator) { currentIndex = n; found = true; break }
                                }
                                if (!found) currentIndex = model.length - 1
                            }
                            onActivated: { if (model[currentIndex].value !== "__custom__") root.cfg_panelSeparator = model[currentIndex].value }
                        }
                        TextField {
                    ...
                    RowLayout {
                        ...
                        spacing: 8
                        SpinBox { from: 0; to: 32; value: root.cfg_panelItemSpacing; onValueModified: root.cfg_panelItemSpacing = value }
                        Label { text: "px"; opacity: 0.65 }
                    }
                    CheckBox {
                        visible: root.cfg_panelInfoMode !== "multiline"
                    ...
                    Platform.FontDialog {
                        ...
                        onAccepted: {
                            ...
                            root.cfg_panelFontSize      = Math.max(6, fontChosen.pointSize > 0
                                                            ? fontChosen.pointSize : 11)
                    ...
                    Label {
                        ...
                        visible: !panelFontSwitch.checked
                    ...
                    ColumnLayout {
                        Kirigami.FormData.label: ""
                        visible: panelFontSwitch.checked && root.cfg_panelFontFamily.length > 0
                        spacing: 2
                        Label {
                            // fontChosen.pointSize always reflects what was chosen in
                            // the dialog; cfg_panelFontSize can be 0 before first open.
                            text: i18nc("@info %1 size %2 family", "%1pt %2",
                                        panelFontDialog.fontChosen.pointSize > 0
                                            ? panelFontDialog.fontChosen.pointSize
                                            : (root.cfg_panelFontSize > 0 ? root.cfg_panelFontSize : 11),
                                        root.cfg_panelFontFamily)
                    ...
                    RowLayout {
                        ...
                        ComboBox {
                            ...
                            textRole: "text"
                            model: [
                                { text: i18n("Font icons (default)"),  value: "wi-font"    },
                            ...
                            Component.onCompleted: {
                                for (var i = 0; i < model.length; ++i)
                                    if (model[i].value === root.cfg_panelIconTheme) { currentIndex = i; break }
                            }
                        ...
                        Label {
                            text: i18n("Size:")
                            visible: iconThemeCombo.model[iconThemeCombo.currentIndex].value !== "wi-font"
                            opacity: 0.8
                        }
                        ComboBox {
                            id: iconSizeCombo
                            ...
                            Component.onCompleted: {
                                for (var i = 0; i < model.length; ++i)
                                    if (model[i].value === root.cfg_panelIconSize) { currentIndex = i; break }
                                if (currentIndex < 0) currentIndex = 1  // default 22
                            }
                    ...
                    RowLayout {
                        Kirigami.FormData.label: ""
                        visible: iconThemeCombo.model[iconThemeCombo.currentIndex].value === "custom"
                        spacing: Kirigami.Units.largeSpacing
                        ColumnLayout {
                            spacing: Kirigami.Units.smallSpacing
                            Label {
                        ...
                        Button {
                            ...
                            onClicked: { root.initPanelModel(); stack.push(panelItemsSubPage) }
                        }
                    }
                    ...
                    Item {
                        ...
                        RowLayout {
                            id: panelPreviewRow; spacing: 10
                            Flow {
                                spacing: 4; Layout.maximumWidth: 260
                                Repeater {
                                    model: root.cfg_panelItemOrder.split(";").filter(function(t) { return t.length > 0 })
                                    delegate: Rectangle {
                                        radius: 3; color: Qt.rgba(1,1,1,0.10)
                                        ...
                                        Label {
                                            ...
                                            text: {
                                                ...
                                                for (var i = 0; i < root.allPanelItemDefs.length; ++i)
                                                    if (root.allPanelItemDefs[i].itemId === d) return root.allPanelItemDefs[i].label
                                                return d
                                            }
                                        }
                                    }
                            ...
                            Button {
                                text: i18n("Configure\u2026"); icon.name: "configure"
                                onClicked: { root.initPanelModel(); stack.push(panelItemsSubPage) }
                            }
                        }
                    }
                ...
                Kirigami.FormLayout {
                    ...
                    SpinBox {
                        ...
                        from: 3; to: 7; value: root.cfg_forecastDays
                    ...
                    RowLayout {
                        ...
                        ComboBox {
                            ...
                            Component.onCompleted: {
                                ...
                                if (theme === "kde" || theme === "wi-font") theme = "symbolic"
                                for (var i = 0; i < model.length; ++i)
                                    if (model[i].value === theme) { currentIndex = i; break }
                    ...
                    Item {
                        ...
                        RowLayout {
                            id: detailsPreviewRow; spacing: 10
                            Flow {
                                spacing: 4; Layout.maximumWidth: 260
                                Repeater {
                                    model: root.cfg_widgetDetailsOrder.split(";").filter(function(t) { return t.length > 0 })
                                    delegate: Rectangle {
                                        radius: 3; color: Qt.rgba(1,1,1,0.10)
                                        border.color: Qt.rgba(1,1,1,0.22); border.width: 1
                                        ...
                                        Label {
                                            id: detailChipLbl; anchors.centerIn: parent
                                            text: {
                                                var d = modelData.trim()
                                                for (var i = 0; i < root.allDetailsDefs.length; ++i)
                                                    if (root.allDetailsDefs[i].itemId === d) return root.allDetailsDefs[i].label
                                                return d
                                            }
                                        }
                                    }
                            ...
                            Button {
                                text: i18n("Configure\u2026"); icon.name: "configure"
                                onClicked: { root.initDetailsModel(); stack.push(detailsSubPage) }
                            }
                        }
                    }
                ...
                Kirigami.FormLayout {
                    ...
                    // ── Tooltip icon theme selector (hidden in Text mode) ──
                    RowLayout {
                        Kirigami.FormData.label: i18n("Icon theme:")
                        spacing: Kirigami.Units.largeSpacing
                        visible: root.cfg_tooltipUseIcons
                        ComboBox {
                            id: ttIconThemeCombo
                            Layout.preferredWidth: 200
                            textRole: "text"
                            ...
                            Component.onCompleted: {
                                for (var i = 0; i < model.length; ++i)
                                    if (model[i].value === root.cfg_tooltipIconTheme) { currentIndex = i; break }
                            }
                        ...
                        ComboBox {
                            ...
                            Component.onCompleted: {
                                for (var i = 0; i < model.length; ++i)
                                    if (model[i].value === root.cfg_tooltipIconSize) { currentIndex = i; break }
                                if (currentIndex < 0) currentIndex = 1
                            }
                            onActivated: root.cfg_tooltipIconSize = model[currentIndex].value
                        }
                    }
                    // Custom theme hint (hidden in Text mode)
                    RowLayout {
                        Kirigami.FormData.label: ""
                        visible: root.cfg_tooltipUseIcons && ttIconThemeCombo.model[ttIconThemeCombo.currentIndex].value === "custom"
                        spacing: Kirigami.Units.largeSpacing
                        ColumnLayout {
                            spacing: Kirigami.Units.smallSpacing
                            Label {
                                text: i18n("Uses KDE system icons by default. Click the button to customise each item’s icon.")
                                opacity: 0.65; font: Kirigami.Theme.smallFont
                                wrapMode: Text.WordWrap; Layout.maximumWidth: 220
                            }
                        }
                        Button {
                            text: i18n("Set your own icons…")
                            icon.name: "color-picker"
                            onClicked: { root.initTooltipModel(); stack.push(tooltipSubPage) }
                        }
                    }
                    ...
                    Item {
                        ...
                        RowLayout {
                            id: ttPreviewRow; spacing: 10
                            Flow {
                                spacing: 4; Layout.maximumWidth: 260
                                Repeater {
                                    model: root.cfg_tooltipItemOrder.split(";").filter(function(t) { return t.length > 0 })
                                    delegate: Rectangle {
                                        radius: 3; color: Qt.rgba(1,1,1,0.10)
                                        border.color: Qt.rgba(1,1,1,0.22); border.width: 1
                                        ...
                                        Label {
                                            id: ttChipLbl; anchors.centerIn: parent
                                            text: {
                                                var d = modelData.trim()
                                                for (var i = 0; i < root.allTooltipDefs.length; ++i)
                                                    if (root.allTooltipDefs[i].itemId === d) return root.allTooltipDefs[i].label
                                                return d
                                            }
                                        }
                                    }
                ...
                Kirigami.FormLayout {
                    ...
                    Kirigami.Separator { Kirigami.FormData.label: i18n("Display"); Kirigami.FormData.isSection: true }
                    RowLayout {
                        Kirigami.FormData.label: i18n("Round values:"); spacing: 12
                        Switch {
                    ...
                    Kirigami.Separator { Kirigami.FormData.label: i18n("Units"); Kirigami.FormData.isSection: true }
                    ComboBox {
                        id: unitsModeCombo
                        Kirigami.FormData.label: i18n("Unit preset:")
                        Layout.preferredWidth: 270
                        model: [
                            { text: i18n("Metric (°C, km/h, hPa, mm)"),         value: "metric"   },
                            { text: i18n("Imperial (°F, mph, inHg, in)"),        value: "imperial" },
                            { text: i18n("Use KDE locale settings"),             value: "kde"      },
                            { text: i18n("Custom (set each unit manually)"),     value: "custom"   }
                        ]
                        Component.onCompleted: {
                            for (var i = 0; i < model.length; ++i)
                                if (model[i].value === root.cfg_unitsMode) { currentIndex = i; break }
                        }
                                                textRole: "text"
                        onActivated: {
                            ...
                            if (mode === "metric") {
                                root.cfg_temperatureUnit = "C"; root.cfg_windSpeedUnit = "kmh"; root.cfg_pressureUnit = "hPa"; root.cfg_precipitationUnit = "mm"
                                setCombo(tempUnitCombo, "C"); setCombo(windUnitCombo, "kmh"); setCombo(pressUnitCombo, "hPa")
                            } else if (mode === "imperial") {
                                root.cfg_temperatureUnit = "F"; root.cfg_windSpeedUnit = "mph"; root.cfg_pressureUnit = "inHg"; root.cfg_precipitationUnit = "in"
                                setCombo(tempUnitCombo, "F"); setCombo(windUnitCombo, "mph"); setCombo(pressUnitCombo, "inHg")
                            } else if (mode === "kde") {
                    ...
                    Kirigami.Separator { Kirigami.FormData.label: i18n("Individual units"); Kirigami.FormData.isSection: true }
                    ComboBox {
                        id: tempUnitCombo
                        ...
                        Layout.preferredWidth: 200
                        model: [
                            { text: i18n("Celsius (°C)"),    value: "C" },
                            { text: i18n("Fahrenheit (°F)"), value: "F" }
                        ]
                        Component.onCompleted: {
                            for (var i = 0; i < model.length; ++i)
                                if (model[i].value === root.cfg_temperatureUnit) { currentIndex = i; break }
                        }
                                                textRole: "text"
                        onActivated: if (root.cfg_unitsMode === "custom") root.cfg_temperatureUnit = model[currentIndex].value
                    }
                    ComboBox {
                        id: windUnitCombo
                        ...
                        Component.onCompleted: {
                            for (var i = 0; i < model.length; ++i)
                                if (model[i].value === root.cfg_windSpeedUnit) { currentIndex = i; break }
                        }
                                                textRole: "text"
                        onActivated: if (root.cfg_unitsMode === "custom") root.cfg_windSpeedUnit = model[currentIndex].value
                    }
                    ComboBox {
                        id: pressUnitCombo
                        ...
                        Component.onCompleted: {
                            for (var i = 0; i < model.length; ++i)
                                if (model[i].value === root.cfg_pressureUnit) { currentIndex = i; break }
                        }
                                                textRole: "text"
                        onActivated: if (root.cfg_unitsMode === "custom") root.cfg_pressureUnit = model[currentIndex].value
                    }
                    Label {
                        Kirigami.FormData.label: ""
                        text: i18n("Individual dropdowns are editable only in Custom mode.\nOther presets set units automatically.")
                        wrapMode: Text.WordWrap; opacity: 0.65; font: Kirigami.Theme.smallFont
    ...
    Component {
        ...
        ColumnLayout {
            ...
            Dialog {
                ...
                standardButtons: Dialog.NoButton
                Label { text: i18n("Keep the changes you made to Panel Items?"); wrapMode: Text.WordWrap }
                footer: DialogButtonBox {
                    Button { text: i18n("Keep Changes"); DialogButtonBox.buttonRole: DialogButtonBox.AcceptRole
                        onClicked: { panelLeaveDialog.accept(); stack.pop() } }
                    Button { text: i18n("Discard"); DialogButtonBox.buttonRole: DialogButtonBox.DestructiveRole
                        onClicked: {
                            root.cfg_panelItemOrder = panelSubPageRoot._savedOrder
                            root.cfg_panelItemIcons = panelSubPageRoot._savedIcons
                            panelLeaveDialog.close(); stack.pop()
                        } }
                    Button { text: i18n("Cancel"); DialogButtonBox.buttonRole: DialogButtonBox.RejectRole
                        onClicked: panelLeaveDialog.close() }
                }
            }

            RowLayout {
                Layout.fillWidth: true; Layout.topMargin: 4; Layout.leftMargin: 4; Layout.rightMargin: 8; Layout.bottomMargin: 4; spacing: 4
                Button {
                    icon.name: "go-previous"; text: i18n("Back"); flat: true
                    onClicked: {
                        // Show confirm if anything changed from the snapshot
                        if (root.cfg_panelItemOrder !== panelSubPageRoot._savedOrder
                         || root.cfg_panelItemIcons !== panelSubPageRoot._savedIcons)
                            panelLeaveDialog.open()
                        else stack.pop()
                    }
                }
                Label { Layout.fillWidth: true; text: i18n("Panel Items"); font.bold: true }
            }
            Kirigami.Separator { Layout.fillWidth: true }
            ScrollView {
                Layout.fillWidth: true; Layout.fillHeight: true; clip: true; contentWidth: availableWidth
                ListView {
                    id: panelItemList; width: parent.width; implicitHeight: contentHeight; clip: true; spacing: 0
                    model: panelWorkingModel
                    highlightMoveDuration: Kirigami.Units.longDuration
                    displaced: Transition { YAnimator { duration: Kirigami.Units.longDuration } }
                    section.property: "itemEnabled"; section.criteria: ViewSection.FullString
                    section.delegate: Kirigami.ListSectionHeader {
                        required property string section; width: panelItemList.width
                        label: section === "true" ? i18n("Enabled") : i18n("Available")
                    }
                    delegate: Item {
                        id: panelDelegateRoot
                        property bool settingsExpanded: false
                        width: panelItemList.width; implicitHeight: panelDelegateCol.implicitHeight
                        ColumnLayout { id: panelDelegateCol; spacing: 0; width: parent.width
                            ItemDelegate {
                                id: panelRowDelegate; Layout.fillWidth: true; hoverEnabled: true; down: false
                                contentItem: RowLayout {
                                    spacing: Kirigami.Units.smallSpacing
                                    Kirigami.ListItemDragHandle {
                                        listItem: panelRowDelegate; listView: panelItemList
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.0
                                        onMoveRequested: function(oldIndex, newIndex) {
                                            ...
                                            if (clamped !== oldIndex) panelWorkingModel.move(oldIndex, clamped, 1)
                                        }
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            ...
                                            visible: root.cfg_panelIconTheme === "wi-font"
                                                  && model.itemWiChar.length > 0
                                        ...
                                        Kirigami.Icon {
                                            ...
                                            visible: (root.cfg_panelIconTheme === "wi-font"
                                                      && (model.itemWiChar.length === 0 || wiFont.status !== FontLoader.Ready))
                                                  || root.cfg_panelIconTheme === "kde"
                                        ...
                                        Kirigami.Icon {
                                            ...
                                            visible: root.cfg_panelIconTheme !== "wi-font"
                                                  && root.cfg_panelIconTheme !== "kde"
                                                  && root.cfg_panelIconTheme !== "custom"
                                                  && root.cfg_panelIconTheme.length > 0
                                            source: {
                                                var th = root.cfg_panelIconTheme
                                                if (!th || th === "wi-font" || th === "kde" || th === "custom") return ""
                                                var sz = root.cfg_panelIconSize || 22;
                                                var b = Qt.resolvedUrl("../icons/" + th + "/" + sz + "/wi-")
                                                var id = model.itemId
                                                if (id === "temperature" || id === "feelslike") return b + "thermometer.svg"
                                                if (id === "humidity")   return b + "humidity.svg"
                                                if (id === "pressure")   return b + "barometer.svg"
                                                if (id === "wind")       return b + "strong-wind.svg"
                                                if (id === "suntimes")   return b + "sunrise.svg"
                                                if (id === "moonphase")  return b + "moon-full.svg"
                                                if (id === "condition")  return b + "day-cloudy.svg"
                                                if (id === "location")   return b + "wind-deg.svg"
                                                return ""
                                            }
                                        ...
                                        Kirigami.Icon {
                                            ...
                                            source: {
                                                ...
                                                if (model.itemId === "condition") {
                                                    ...
                                                    return ("condition-clear" in m && m["condition-clear"].length > 0)
                                                        ? m["condition-clear"] : "weather-clear"
                                                }
                                    ...
                                    ColumnLayout {
                                        ...
                                        Label { Layout.fillWidth: true; text: model.itemLabel; elide: Text.ElideRight; opacity: model.itemEnabled ? 1.0 : 0.55 }
                                        Label { Layout.fillWidth: true; text: model.itemDesc; font: Kirigami.Theme.smallFont; elide: Text.ElideRight; opacity: 0.55 }
                                    }
                                    // Sun times gear (only in non-custom themes — custom uses the configure dialog)
                                    ToolButton {
                                        visible: model.itemId === "suntimes"
                                                 && root.cfg_panelIconTheme !== "custom"
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.3
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        icon.name: "configure"; checkable: true; checked: panelDelegateRoot.settingsExpanded
                                        ToolTip.visible: hovered; ToolTip.text: i18n("Sun times options")
                                    ...
                                    ToolButton {
                                        ...
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        icon.name: "color-picker"
                                        ToolTip.visible: hovered; ToolTip.text: i18n("Configure icon…")
                                        onClicked: {
                                            if (model.itemId === "condition") {
                                                ...
                                                for (var i = 0; i < sunModeDialogCombo.model.length; ++i)
                                                    if (sunModeDialogCombo.model[i].value === root.cfg_panelSunTimesMode)
                                                        { sunModeDialogCombo.currentIndex = i; break }
                                    ...
                                    ToolButton {
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.25
                                        icon.name: model.itemShowIcon ? "view-visible" : "view-hidden"
                                        ToolTip.visible: hovered; ToolTip.text: model.itemShowIcon ? i18n("Hide prefix icon") : i18n("Show prefix icon")
                                        onClicked: { panelWorkingModel.setProperty(model.index, "itemShowIcon", !model.itemShowIcon); root.applyPanelItems() }
                                    }
                                    // Enable/disable (font-enable / font-disable)
                                    ToolButton {
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        icon.name: model.itemEnabled ? "font-enable" : "font-disable"
                                        ToolTip.visible: hovered; ToolTip.text: model.itemEnabled ? i18n("Disable item") : i18n("Enable item")
                                        onClicked: {
                                            var idx = model.index; var nowOn = !model.itemEnabled
                                            if (!nowOn) panelDelegateRoot.settingsExpanded = false
                                            panelWorkingModel.setProperty(idx, "itemEnabled", nowOn)
                                            var boundary = root.firstPanelDisabledIndex()
                                            if (nowOn) { if (boundary > 0 && idx >= boundary) panelWorkingModel.move(idx, boundary - 1, 1) }
                                            else {
                                                var lastEnabled = -1
                                                for (var i = 0; i < panelWorkingModel.count; ++i) if (panelWorkingModel.get(i).itemEnabled) lastEnabled = i
                                                if (lastEnabled >= 0 && idx <= lastEnabled) panelWorkingModel.move(idx, lastEnabled, 1)
                                                else if (lastEnabled < 0 && idx > 0) panelWorkingModel.move(idx, 0, 1)
                                            }
                            ...
                            RowLayout {
                                visible: model.itemId === "suntimes"
                                         && panelDelegateRoot.settingsExpanded
                                         && root.cfg_panelIconTheme !== "custom"
                                Layout.fillWidth: true
                                Layout.leftMargin: Kirigami.Units.iconSizes.smallMedium * 2 + Kirigami.Units.largeSpacing * 2
                                Layout.rightMargin: Kirigami.Units.largeSpacing; Layout.bottomMargin: Kirigami.Units.smallSpacing; spacing: Kirigami.Units.largeSpacing
                                Label { text: i18n("Sun times mode:"); font: Kirigami.Theme.smallFont; opacity: 0.8 }
                                ComboBox {
                                    id: sunTimesInlineCombo; Layout.fillWidth: true
                                    model: [
                                        { text: i18n("Upcoming (next sunrise or sunset)"), value: "upcoming" },
                                        { text: i18n("Both  07:17 / 18:03"),               value: "both"     },
                                        { text: i18n("Sunrise only  07:17"),               value: "sunrise"  },
                                    ...
                                    textRole: "text"
                                    Component.onCompleted: { for (var i = 0; i < model.length; ++i) if (model[i].value === root.cfg_panelSunTimesMode) { currentIndex = i; break } }
                                    onActivated: root.cfg_panelSunTimesMode = model[currentIndex].value
                                }
                            }
            ...
            ColumnLayout {
                ...
                Label { text: i18n("Button guide"); font.bold: true; opacity: 0.85 }
                ...
                Flow {
                    ...
                    RowLayout {
                        ...
                        Label { text: i18n("Drag to reorder enabled items"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    ...
                    RowLayout {
                        ...
                        Label { text: i18n("Choose a custom icon for this item"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    ...
                    RowLayout {
                        ...
                        Label { text: i18n("Show / hide the prefix icon on the panel"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    ...
                    RowLayout {
                        ...
                        Label { text: i18n("Enable or disable this item on the panel"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                }
            }
        }
    ...
    Component {
        ...
        ColumnLayout {
            ...
            Dialog {
                ...
                modal: true; parent: Overlay.overlay; anchors.centerIn: parent
                standardButtons: Dialog.NoButton
                Label { text: i18n("Keep the changes you made to Details Items?"); wrapMode: Text.WordWrap }
                footer: DialogButtonBox {
                    Button { text: i18n("Keep Changes"); DialogButtonBox.buttonRole: DialogButtonBox.AcceptRole
                        onClicked: { detailsLeaveDialog.accept(); stack.pop() } }
                    Button { text: i18n("Discard"); DialogButtonBox.buttonRole: DialogButtonBox.DestructiveRole
                        onClicked: {
                            root.cfg_widgetDetailsOrder = detailsSubPageRoot._savedOrder
                            detailsLeaveDialog.close(); stack.pop()
                        } }
                    Button { text: i18n("Cancel"); DialogButtonBox.buttonRole: DialogButtonBox.RejectRole
            ...
            RowLayout {
                ...
                Label { Layout.fillWidth: true; text: i18n("Details Items"); font.bold: true }
            }
            Kirigami.Separator { Layout.fillWidth: true }
            ScrollView {
                Layout.fillWidth: true; Layout.fillHeight: true; clip: true; contentWidth: availableWidth
                ListView {
                    id: detailsList; width: parent.width; implicitHeight: contentHeight; clip: true; spacing: 0
                    model: detailsWorkingModel
                    highlightMoveDuration: Kirigami.Units.longDuration
                    displaced: Transition { YAnimator { duration: Kirigami.Units.longDuration } }
                    section.property: "itemEnabled"; section.criteria: ViewSection.FullString
                    section.delegate: Kirigami.ListSectionHeader {
                        required property string section; width: detailsList.width
                        label: section === "true" ? i18n("Shown") : i18n("Hidden")
                    }
                    delegate: Item {
                        id: detailsDelegateRoot
                        width: detailsList.width; implicitHeight: detailsDelegateCol.implicitHeight
                        ColumnLayout { id: detailsDelegateCol; spacing: 0; width: parent.width
                            ItemDelegate {
                                id: detailsRowDelegate; Layout.fillWidth: true; hoverEnabled: true; down: false
                                contentItem: RowLayout {
                                    spacing: Kirigami.Units.smallSpacing
                                    // ── Drag handle (only active for enabled items) ──────
                                    Kirigami.ListItemDragHandle {
                                        listItem: detailsRowDelegate; listView: detailsList
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.0
                                        onMoveRequested: function(oldIndex, newIndex) {
                                            var boundary = root.firstDetailsDisabledIndex()
                                            var clamped = (boundary < 0) ? newIndex : Math.min(newIndex, boundary - 1)
                                            if (clamped !== oldIndex) detailsWorkingModel.move(oldIndex, clamped, 1)
                                        }
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            ...
                                            font.pixelSize: Kirigami.Units.iconSizes.smallMedium - 2
                                            color: Kirigami.Theme.textColor
                                            visible: root.cfg_widgetIconTheme === "wi-font"
                                                  && model.itemWiChar.length > 0
                                                  && wiFont.status === FontLoader.Ready
                                        }
                                        // KDE / wi-font fallback
                                        Kirigami.Icon {
                                            anchors.fill: parent; source: model.itemFallback
                                            visible: (root.cfg_widgetIconTheme === "wi-font"
                                                      && (model.itemWiChar.length === 0 || wiFont.status !== FontLoader.Ready))
                                                  || root.cfg_widgetIconTheme === "kde"
                                        }
                                        // SVG theme icon (symbolic / flat-color / 3d-oxygen)
                                        Kirigami.Icon {
                                            anchors.fill: parent
                                            visible: root.cfg_widgetIconTheme !== "wi-font"
                                                  && root.cfg_widgetIconTheme !== "kde"
                                                  && root.cfg_widgetIconTheme.length > 0
                                            source: {
                                                var th = root.cfg_widgetIconTheme
                                                if (!th || th === "wi-font" || th === "kde") return ""
                                                var b = Qt.resolvedUrl("../icons/" + th + "/16/wi-")
                                                var id = model.itemId
                                                if (id === "feelslike" || id === "dewpoint") return b + "thermometer.svg"
                                                if (id === "humidity")   return b + "humidity.svg"
                                                if (id === "pressure")   return b + "barometer.svg"
                                                if (id === "wind")       return b + "strong-wind.svg"
                                                if (id === "suntimes")   return b + "sunrise.svg"
                                                if (id === "moonphase")  return b + "moon-full.svg"
                                                if (id === "visibility") return b + "fog.svg"
                                                return ""
                                            }
                                    ...
                                    ToolButton {
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        icon.name: model.itemEnabled ? "font-enable" : "font-disable"
                                        ToolTip.visible: hovered; ToolTip.text: model.itemEnabled ? i18n("Hide from details") : i18n("Show in details")
                                        onClicked: {
                                            var idx = model.index; var nowOn = !model.itemEnabled
                                            ...
                                            if (nowOn) {
                                                ...
                                                var lastEnabled = -1
                                                for (var i = 0; i < detailsWorkingModel.count; ++i)
                                                    if (detailsWorkingModel.get(i).itemEnabled) lastEnabled = i
            ...
            ColumnLayout {
                ...
                Flow {
                    ...
                    RowLayout { spacing: 4
                        Kirigami.Icon { source: "handle-sort"; implicitWidth: Kirigami.Units.iconSizes.small; implicitHeight: Kirigami.Units.iconSizes.small }
                        Label { text: i18n("Drag to reorder shown items"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    RowLayout { spacing: 4
                        Kirigami.Icon { source: "font-enable"; implicitWidth: Kirigami.Units.iconSizes.small; implicitHeight: Kirigami.Units.iconSizes.small }
                        Label { text: i18n("Show or hide this detail item"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                }
            }
        }
    ...
    Component {
        ...
        ColumnLayout {
            ...
            Dialog {
                ...
                modal: true; parent: Overlay.overlay; anchors.centerIn: parent
                standardButtons: Dialog.NoButton
                Label { text: i18n("Keep the changes you made to Tooltip Items?"); wrapMode: Text.WordWrap }
                footer: DialogButtonBox {
                    Button { text: i18n("Keep Changes"); DialogButtonBox.buttonRole: DialogButtonBox.AcceptRole
                        onClicked: { ttLeaveDialog.accept(); stack.pop() } }
                    Button { text: i18n("Discard"); DialogButtonBox.buttonRole: DialogButtonBox.DestructiveRole
                        onClicked: {
                            root.cfg_tooltipItemOrder = ttSubPageRoot._savedOrder
                            root.cfg_tooltipItemIcons = ttSubPageRoot._savedIcons
                            ttLeaveDialog.close(); stack.pop()
                        } }
                    Button { text: i18n("Cancel"); DialogButtonBox.buttonRole: DialogButtonBox.RejectRole
            ...
            RowLayout {
                ...
                Button {
                    icon.name: "go-previous"; text: i18n("Back"); flat: true
                    onClicked: {
                        if (root.cfg_tooltipItemOrder !== ttSubPageRoot._savedOrder
                         || root.cfg_tooltipItemIcons !== ttSubPageRoot._savedIcons)
                            ttLeaveDialog.open()
                        else stack.pop()
                    }
                }
                Label { Layout.fillWidth: true; text: i18n("Tooltip Items"); font.bold: true }
            }
            Kirigami.Separator { Layout.fillWidth: true }
            ScrollView {
                Layout.fillWidth: true; Layout.fillHeight: true; clip: true; contentWidth: availableWidth
                ListView {
                    id: tooltipItemList; width: parent.width; implicitHeight: contentHeight; clip: true; spacing: 0
                    model: tooltipWorkingModel
                    highlightMoveDuration: Kirigami.Units.longDuration
                    displaced: Transition { YAnimator { duration: Kirigami.Units.longDuration } }
                    section.property: "itemEnabled"; section.criteria: ViewSection.FullString
                    section.delegate: Kirigami.ListSectionHeader {
                        required property string section; width: tooltipItemList.width
                        label: section === "true" ? i18n("Enabled") : i18n("Available")
                    }
                    delegate: Item {
                        id: ttDelegateRoot
                        property bool settingsExpanded: false
                        width: tooltipItemList.width; implicitHeight: ttDelegateCol.implicitHeight
                        ColumnLayout { id: ttDelegateCol; spacing: 0; width: parent.width
                            ItemDelegate {
                                id: ttRowDelegate; Layout.fillWidth: true; hoverEnabled: true; down: false
                                contentItem: RowLayout {
                                    spacing: Kirigami.Units.smallSpacing
                                    // Drag handle
                                    Kirigami.ListItemDragHandle {
                                        listItem: ttRowDelegate; listView: tooltipItemList
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.0
                                        onMoveRequested: function(oldIndex, newIndex) {
                                            var boundary = root.firstTooltipDisabledIndex()
                                            var clamped = (boundary < 0) ? newIndex : Math.min(newIndex, boundary - 1)
                                            if (clamped !== oldIndex) tooltipWorkingModel.move(oldIndex, clamped, 1)
                                        }
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            ...
                                            font.pixelSize: Kirigami.Units.iconSizes.smallMedium - 2
                                            color: Kirigami.Theme.textColor
                                            visible: root.cfg_tooltipIconTheme === "wi-font"
                                                  && model.itemWiChar.length > 0
                                                  && wiFont.status === FontLoader.Ready
                                        }
                                        // KDE / fallback icon
                                        ...
                                        Kirigami.Icon {
                                            ...
                                            visible: root.cfg_tooltipIconTheme !== "wi-font"
                                                  && root.cfg_tooltipIconTheme !== "kde"
                                                  && root.cfg_tooltipIconTheme !== "custom"
                                                  && root.cfg_tooltipIconTheme.length > 0
                                            source: {
                                    ...
                                    ToolButton {
                                        ...
                                        enabled: model.itemEnabled; opacity: model.itemEnabled ? 1.0 : 0.3
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        icon.name: "configure"; checkable: true; checked: ttDelegateRoot.settingsExpanded
                                        ToolTip.visible: hovered; ToolTip.text: i18n("Sun times options")
                                    ...
                                    ToolButton {
                                        ...
                                        icon.name: model.itemShowIcon ? "view-visible" : "view-hidden"
                                        ToolTip.visible: hovered; ToolTip.text: model.itemShowIcon ? i18n("Hide prefix icon") : i18n("Show prefix icon")
                                        onClicked: { tooltipWorkingModel.setProperty(model.index, "itemShowIcon", !model.itemShowIcon); root.applyTooltipItems() }
                                    }
                                    // Enable / disable
                                    ToolButton {
                                        implicitWidth: Kirigami.Units.iconSizes.medium; implicitHeight: Kirigami.Units.iconSizes.medium
                                        ...
                                        onClicked: {
                                            var idx = model.index; var nowOn = !model.itemEnabled
                                            if (!nowOn) ttDelegateRoot.settingsExpanded = false
                                            tooltipWorkingModel.setProperty(idx, "itemEnabled", nowOn)
                                            var boundary = root.firstTooltipDisabledIndex()
                                            if (nowOn) { if (boundary > 0 && idx >= boundary) tooltipWorkingModel.move(idx, boundary - 1, 1) }
                                            else {
                                                var lastEnabled = -1
                                                for (var i = 0; i < tooltipWorkingModel.count; ++i) if (tooltipWorkingModel.get(i).itemEnabled) lastEnabled = i
                                                if (lastEnabled >= 0 && idx <= lastEnabled) tooltipWorkingModel.move(idx, lastEnabled, 1)
                                                else if (lastEnabled < 0 && idx > 0) tooltipWorkingModel.move(idx, 0, 1)
                                            }
                            ...
                            RowLayout {
                                ...
                                ComboBox {
                                    ...
                                    textRole: "text"
                                    Component.onCompleted: { for (var i = 0; i < model.length; ++i) if (model[i].value === root.cfg_tooltipSunTimesMode) { currentIndex = i; break } }
            ...
            ColumnLayout {
                ...
                Flow {
                    ...
                    RowLayout { visible: root.cfg_tooltipIconTheme === "custom"; spacing: 4
                        Kirigami.Icon { source: "color-picker"; implicitWidth: Kirigami.Units.iconSizes.small; implicitHeight: Kirigami.Units.iconSizes.small }
                        Label { text: i18n("Choose a custom icon for this item"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    RowLayout { spacing: 4
                        Kirigami.Icon { source: "view-visible"; implicitWidth: Kirigami.Units.iconSizes.small; implicitHeight: Kirigami.Units.iconSizes.small }
                        Label { text: i18n("Show / hide the prefix icon"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                    RowLayout { spacing: 4
                        Kirigami.Icon { source: "font-enable"; implicitWidth: Kirigami.Units.iconSizes.small; implicitHeight: Kirigami.Units.iconSizes.small }
                        Label { text: i18n("Enable or disable this item"); font: Kirigami.Theme.smallFont; opacity: 0.75 }
                    }
                }
            }
        }
