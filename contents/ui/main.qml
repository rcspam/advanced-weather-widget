...
PlasmoidItem {
    ...
    readonly property bool hasSelectedTown:
        (Plasmoid.configuration.locationName || "").trim().length > 0 ||
        (Plasmoid.configuration.autoDetectLocation &&
         (Plasmoid.configuration.latitude  !== 0.0 ||
          Plasmoid.configuration.longitude !== 0.0))
    ...
    function _autoReverseGeocode(lat, lon) {
        ...
        req.setRequestHeader("User-Agent",
            "P4PWeatherWidget/1.0 (KDE Plasma plasmoid)")
    ...
    function refreshWeather() { weatherService.refreshNow(); }

    /** Fetch hourly data for a specific date — called by FullView */
    function fetchHourlyForDate(dateStr) { weatherService.fetchHourlyForDate(dateStr); }
    ...
    function tempValue(celsius) {
        return W.formatTemp(celsius,
            Plasmoid.configuration.temperatureUnit || "C",
            Plasmoid.configuration.roundValues);
    }
    ...
    function weatherCodeToText(code, night) {
        var n = (night === true);
        switch (code) {
            case 0:  return n ? i18n("Clear night")               : i18n("Clear sky");
            case 1:  return i18n("Mainly clear");
            case 2:  return i18n("Partly cloudy");
            case 3:  return i18n("Overcast");
            case 45: return i18n("Fog");
            case 48: return i18n("Rime fog");
            case 51: return i18n("Light drizzle");
            case 53: return i18n("Drizzle");
            case 55: return i18n("Heavy drizzle");
            case 56: return i18n("Light freezing drizzle");
            case 57: return i18n("Freezing drizzle");
            case 61: return i18n("Light rain");
            case 63: return i18n("Rain");
            case 65: return i18n("Heavy rain");
            case 66: return i18n("Light freezing rain");
            case 67: return i18n("Freezing rain");
            case 71: return i18n("Light snow");
            case 73: return i18n("Snow");
            case 75: return i18n("Heavy snow");
            case 77: return i18n("Snow grains");
            case 80: return i18n("Light showers");
            case 81: return i18n("Showers");
            case 82: return i18n("Heavy showers");
            case 85: return i18n("Light snow showers");
            case 86: return i18n("Snow showers");
            case 95: return i18n("Thunderstorm");
            case 96: return i18n("Thunderstorm with hail");
            case 99: return i18n("Heavy thunderstorm with hail");
            default: return i18n("Partly cloudy");
        }
    }
    ...
    function moonPhaseLabel() { return i18n(Moon.moonPhaseNameKey()); }
    function moonPhaseGlyph() { return Moon.moonPhaseGlyph(); }
    ...
    function panelItemGlyph(tok) {
        if (tok === "temperature") return "\uF055";        // wi-thermometer
        if (tok === "feelslike")   return "\uF055";        // wi-thermometer
        if (tok === "humidity")    return "\uF07A";        // wi-humidity
        if (tok === "pressure")    return "\uF079";        // wi-barometer
        if (tok === "location")    return "\uF0B1";       // wi-direction (F0B1)
        if (tok === "moonphase")   return moonPhaseGlyph();
        if (tok === "wind")        return W.windDirectionGlyph(windDirection);
        if (tok === "condition") {
        ...
        if (tok === "suntimes") {
            ...
            if (mode === "upcoming") {
                if (riseMins >= 0 && nowMins < riseMins) return "\uF051";
                if (setMins  >= 0 && nowMins < setMins)  return "\uF052";
            }
            return "\uF051";
        }
        return "";
    ...
    function panelItemIconInfo(tok) {
        ...
        if (theme === "custom") {
            ...
            var defaults = {
                ...
                temperature: "thermometer", feelslike: "thermometer",
                humidity:    "weather-showers", pressure: "weather-overcast",
                wind:        "weather-windy",
                moonphase:   "weather-clear-night", location: "mark-location"
            };
            ...
            if (tok === "condition") {
                if (customMap["condition-custom"] === "1") {
                    ...
                    var condKey;
                    if (code2 === 0)             condKey = night2 ? "condition-clear-night"  : "condition-clear";
                    else if (code2 <= 2)         condKey = night2 ? "condition-cloudy-night" : "condition-cloudy-day";
                    else if (code2 === 3)        condKey = "condition-overcast";
                    else if (code2 <= 48)        condKey = "condition-fog";
                    else if (code2 <= 65)        condKey = "condition-rain";
                    else if (code2 <= 75)        condKey = "condition-snow";
                    else                         condKey = "condition-storm";
                    var fallback2  = W.weatherCodeToIcon(code2, night2);
                    var condSaved2 = (condKey in customMap && customMap[condKey].length > 0)
                                     ? customMap[condKey] : fallback2;
                    return { type: "kde", source: condSaved2 };
                }
                // KDE mode (condition-custom not set) → system icon based on weather code
                return { type: "kde", source: W.weatherCodeToIcon(weatherCode, isNightTime()) };
            }
            ...
            if (tok === "suntimes") {
                ...
                var useSet2 = (mode2 === "sunset") ||
                              (mode2 === "upcoming" && riseM2 >= 0 && nowM2 >= riseM2 &&
                               (setM2 < 0 || nowM2 < setM2));
                var sunKey2   = useSet2 ? "suntimes-sunset" : "suntimes-sunrise";
                var sunDef2   = useSet2 ? "weather-sunset" : "weather-sunrise";
                var sunSaved2 = (sunKey2 in customMap && customMap[sunKey2].length > 0)
                                ? customMap[sunKey2] : sunDef2;
                return { type: "kde", source: sunSaved2 };
            }

            var iconName = (tok in customMap && customMap[tok].length > 0)
                ? customMap[tok]
                : (tok in defaults ? defaults[tok] : "");
            return { type: "kde", source: iconName };
        }

        // ── KDE system theme ──────────────────────────────────────────────────
        if (theme === "kde") {
            if (tok === "temperature" || tok === "feelslike")
                return { type: "kde", source: "temperature" };
            if (tok === "humidity")   return { type: "kde", source: "weather-humidity"         };
            if (tok === "pressure")   return { type: "kde", source: "weather-pressure"         };
            if (tok === "wind")       return { type: "kde", source: "weather-wind-beaufort-0"  };
            if (tok === "condition")  return { type: "kde", source: W.weatherCodeToIcon(weatherCode, isNightTime()) };
            if (tok === "suntimes") {
                ...
                var useSet3  = (mode3 === "sunset") ||
                               (mode3 === "upcoming" && riseM3 >= 0 && nowM3 >= riseM3 && (setM3 < 0 || nowM3 < setM3));
                return { type: "kde", source: useSet3 ? "weather-sunset" : "weather-sunrise" };
            }
            if (tok === "moonphase") return { type: "kde", source: "weather-clear-night"  };
            if (tok === "location")  return { type: "kde", source: "mark-location"        };
            return { type: "kde", source: "" };
        }
        ...
        if (tok === "temperature" || tok === "feelslike")
            return { type: "svg", source: base + "thermometer.svg" };
        if (tok === "humidity")
            return { type: "svg", source: base + "humidity.svg" };
        if (tok === "pressure")
            return { type: "svg", source: base + "barometer.svg" };
        if (tok === "wind")
            return { type: "svg", source: base + "strong-wind.svg" };
        // Use wi-wind-deg.svg as location icon for all SVG themes
        if (tok === "location")
            return { type: "svg", source: base + "wind-deg.svg" };
        if (tok === "condition") {
            var code = weatherCode;
            var night = isNightTime();
            var stem;
            if (code === 0)      stem = night ? "night-clear"               : "day-sunny";
            else if (code <= 2)  stem = night ? "night-alt-partly-cloudy"   : "day-cloudy";
            else if (code === 3) stem = "cloudy";
            else if (code <= 48) stem = night ? "night-fog"                 : "day-fog";
            else if (code <= 65) stem = night ? "night-alt-rain"            : "day-rain";
            else if (code <= 75) stem = night ? "night-alt-snow"            : "day-snow";
            else                 stem = night ? "night-alt-thunderstorm"    : "day-thunderstorm";
            return { type: "svg", source: base + stem + ".svg" };
        }
        if (tok === "suntimes") {
            var mode2   = Plasmoid.configuration.panelSunTimesMode || "upcoming";
            if (mode2 === "sunset")  return { type: "svg", source: base + "sunset.svg"  };
            if (mode2 === "sunrise") return { type: "svg", source: base + "sunrise.svg" };
            if (mode2 === "both")    return { type: "svg", source: base + "sunrise.svg" }; // CompactView handles both-mode split
            // "upcoming": pick based on current time
            var nowM2   = (new Date()).getHours() * 60 + (new Date()).getMinutes();
            var riseM2  = parseSunTimeMins(sunriseTimeText);
            var setM2   = parseSunTimeMins(sunsetTimeText);
            var useSet2 = (riseM2 >= 0 && nowM2 >= riseM2 && (setM2 < 0 || nowM2 < setM2));
            return { type: "svg", source: base + (useSet2 ? "sunset" : "sunrise") + ".svg" };
        }
        if (tok === "moonphase")
            // moonPhaseSvgStem() returns e.g. "moon-new" → wi-moon-new.svg
            return { type: "svg", source: base + Moon.moonPhaseSvgStem() + ".svg" };

        return { type: "svg", source: "" };
    }
    ...
    function panelItemTextOnly(tok) {
        var mode = Plasmoid.configuration.panelSunTimesMode || "upcoming";
        if (tok === "location")    return (Plasmoid.configuration.locationName || "").split(",")[0].trim();
        if (tok === "temperature") return tempValue(temperatureC);
        if (tok === "condition")   return weatherCodeToText(weatherCode, isNightTime());
        if (tok === "wind")        return windValue(windKmh);
        if (tok === "feelslike")   return tempValue(apparentC);
        if (tok === "humidity")    return isNaN(humidityPercent) ? "--" : Math.round(humidityPercent) + "%";
        if (tok === "pressure")    return pressureValue(pressureHpa);
        if (tok === "moonphase")   return moonPhaseLabel();
        if (tok === "suntimes") {
            var nowMins  = (new Date()).getHours() * 60 + (new Date()).getMinutes();
            var riseMins = parseSunTimeMins(sunriseTimeText);
            var setMins  = parseSunTimeMins(sunsetTimeText);
            if (mode === "upcoming") {
                if (riseMins >= 0 && nowMins < riseMins) return formatTimeForDisplay(sunriseTimeText);
                if (setMins  >= 0 && nowMins < setMins)  return formatTimeForDisplay(sunsetTimeText);
                return formatTimeForDisplay(sunriseTimeText);
            }
            if (mode === "sunrise") return formatTimeForDisplay(sunriseTimeText);
            if (mode === "sunset")  return formatTimeForDisplay(sunsetTimeText);
    ...
    Timer {
        ...
        running:  true
        repeat:   true
        onTriggered: { var dummy = Moon.getMoonAge(); }
    }
    ...
    Connections {
        ...
        function onLocationNameChanged()    { refreshDebounce.restart() }
        function onLatitudeChanged()        { refreshDebounce.restart() }
        function onLongitudeChanged()       { refreshDebounce.restart() }
        function onTimezoneChanged()        { refreshDebounce.restart() }
        function onWeatherProviderChanged() { refreshDebounce.restart() }
        function onForecastDaysChanged()    { refreshDebounce.restart() }
        function onPanelInfoModeChanged() { root.panelScrollIndex = 0; }
    }
}
